<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class Dynamic extends CI_Controller {

    function __construct()
    {
        parent::__construct();
        $this->load->library('form_validation');
		$this->load->model('user_model');
		$this->load->model('dynamic_model', 'dynamic');
		
        $this->load->helper(array('form', 'url'));
        $this->load->library('session');
        $this->load->library ( 'avatarlib' );
        $this->load->library('detect');
		
        $ci = & get_instance();
        $variable = array('haction'=>'dynamic','titles'=>'丁盯动态-丁盯智能官网');
        $ci->load->vars($variable);
    }
    
	public function index(){
		$user_id = $this->session->userdata('user_id');
	    $info = $this->user_model->get_user(array('id' => $user_id));
		$output['info'] = $info;
		
		//分页开始
		$page = $this->input->get('page');
		$page = empty($page)? 1 : $page;
		
		$alldynamic = $this->dynamic->get_alldynamic($page);
		//p($allmedia);die;
		$ret = $this->dynamic->get_count();
		$output['count'] = isset($ret['num']) ? $ret['num'] : 0;
		//p($output);die;
	
		$output['alldynamic'] = $alldynamic;
		//p($output);die;
		$output['path'] = 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '/pc/dynamic';
		$output['page'] = $page;
		
		//分页结束
		
		$this->load->library('detect');
		$device = $this->detect->is_mobile() ? 'mobile' : 'pc';
		
		if ($device == 'pc') {
		    //$this->load->view('pc/newproduct', $output);
		    $this->load->view('pc/dynamic', $output);
		} else {
		    $this->load->view('m/dynamic', $output);
		}

	}
	public function dynamicDetail(){
	    
	    $id = $this->input->get('id');
	    //p($id);die;
	    $user_id = $this->session->userdata('user_id');
	    $info = $this->user_model->get_user(array('id' => $user_id));
	    $output['info'] = $info;
	    //p($output);die;
	    $dy = $this->dynamic->get_dynamic(array('id' => $id));
	/*
	    $v=$jobs;
	
	    if('0' == $v['type']) $jobs['type'] = "全职";
	    else if('1' == $v['type']) $jobs['type'] = "实习";
	    else $jobs['type'] = "全职/实习";
	*/
	    $output['dynamic'] = $dy;
	    //p($output);die;
	    $this->load->library('detect');
	    $device = $this->detect->is_mobile() ? 'mobile' : 'pc';
	
	    if ($device == 'pc') {
	        $v=$dy;
	        //p($v['video']);die;
	        if(!empty($v['video'])){
	            $this->load->view('pc/dynamicDetail', $output);
	        }else{
	            $this->load->view('pc/dynamicDetail1', $output);
	        }
	        
	    } else {
	        $this->cismarty->display('m/dynamicDetail');
	    }
	
	}
}
